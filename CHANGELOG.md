...
**Languages added:** English, ��������, ���w����, �ձ��Z, ???, Espa?ol, Fran?ais, Deutsch, Portugu��s, ������ܧڧ�, ???????, Ti?ng Vi?t, ???????, Bahasa Indonesia (14 total).
...
### Feature �� Auto-fetch model list from API providers
...
- Two dropdowns: `#modelSelect` for presets, `#customModel` for custom �� never shown simultaneously.

**popup.ts:**
- `fetchModelsForCurrentProvider()` �� sends `fetchModels` action to background, populates the appropriate dropdown.
- Auto-fetches models on popup open if API key is already saved.
- Restores previously selected model after fetch.
- "Manual input..." option at end of dropdown �� prompts user for custom model name.
- Saves selected model to `chrome.storage.local` on Save.

**background.ts:**
- `fetchModels()` dispatcher �� routes to correct API by provider.
- `fetchModelsOpenAI()` �� GET `/models` with Bearer auth (used by DeepSeek, OpenAI, Custom).
- `fetchModelsGemini()` �� GET `v1beta/models?key=`, filters for `generateContent` support.
- `CLAUDE_MODELS` �� hardcoded list (Anthropic has no public model list API).
- `deriveModelsUrl()` �� strips `/chat/completions` suffix from custom base URLs.
...
**Severity:** High �� outgoing translation fails for longer messages like "����һ��������Ϣ".

### Root Cause

1. **Long text translation timeout:** DeepSeek API response time scales with text length. Short text ("�ɹ�") translates in <1s, but longer text ("����һ��������Ϣ") can take 2-3s. The Enter key handler had no explicit timeout �� if the API was slow, `isProcessingSend` would reset prematurely (150ms in `finally`), allowing a second Enter to bypass interception and send the original Chinese text.
...
- Short text ("�ɹ�"): Pre-cached during typing �� Enter �� instant send (0ms API wait).
- Long text ("����һ��������Ϣ"): Enter �� shows blocked Enter attempts in console �� waits for API �� replaces text �� sends English version.
...
2. **Send button not found:** WhatsApp removed `data-testid="send"` from the send button. The button now only has `aria-label="����"` (or "Send" in English locales), with a child span containing `data-testid="wds-ic-send-filled"`.
...
- `findSendButton()`: Reordered selector priority �� `aria-label="����"` is now primary (since it's always present), with `data-testid="send"` as legacy fallback.
...
- Send button found via `aria-label="����"`.
