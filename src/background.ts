...
// Bridges content script ¡ú multiple AI translation APIs
...
function buildPrompt(sourceLang: string, targetLang: string): string {
  ...
  return `You are a professional translator. Translate the following ${source} text into natural, fluent ${target}. Preserve tone (casual, formal, technical). Only respond with the translation ¡ª no explanations, no notes.`;
}
...
// ©¤©¤ Claude (Anthropic) ¡ª different API format ©¤©¤©¤©¤©¤©¤
...
// ©¤©¤ Google Gemini ¡ª also different format ©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤
...
function humanizeError(status: number, body: string): string {
  ...
  switch (status) {
    case 401:
      ...
      break;
    case 429:
      msg = "Rate limited ¡ª retry shortly";
      break;
    case 403:
...
): Promise<{ data: Array<{ id: string; name: string }>; error?: string }> {
  try {
    switch (provider) {
      case "deepseek":
        return await fetchModelsOpenAI("https://api.deepseek.com/v1", apiKey);
      case "openai":
        return await fetchModelsOpenAI("https://api.openai.com/v1", apiKey);
      case "claude":
        return { data: CLAUDE_MODELS };
      case "gemini":
        return await fetchModelsGemini(apiKey);
      case "custom": {
        if (!customBaseUrl)
          return { data: [], error: "No base URL configured" };
        const modelsUrl = deriveModelsUrl(customBaseUrl);
        return await fetchModelsOpenAI(modelsUrl, apiKey);
      }
      default:
        return { data: [], error: "Unknown provider" };
    }
  } catch (err: any) {
